...
const musicaFondo= new Audio("assets/Crimson_Valor_s_Crescendo.mp3");
...
const weapons = [
  { name: 'palo', power: 5 },
  { name: 'daga', power: 30 },
  { name: 'martillo de garras', power: 50 },
  { name: 'espada', power: 100 },
  { name: 'hacha', power: 125 },
  { name: 'maza', power: 190 },
...
function goStore() { 
  ...
  if (currentWeapon < weapons.length - 1) {
    let price = 30 * currentWeapon + 30;
    button2.innerText = "Comprar arma (" + price + " oro)";
  } else {
...
function buyWeapon() {
  if (currentWeapon < weapons.length - 1) {
    ...
    if (gold >= price) {
      ...
      if (currentWeapon < weapons.length - 1) {
        let nextPrice = 30 * currentWeapon + 30;
        button2.innerText = "Comprar arma (" + nextPrice + " oro)";
      } else {
        button2.innerText = "Vender arma por 15 de oro";
        button2.onclick = sellWeapon;
      }
...
function pick(guess) {
  ...
  text.innerText = "Elegiste " + guess + ". Aquí están los números al azar:\n";
  for (let i = 0; i < 10; i++) {
